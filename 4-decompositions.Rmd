...
holidays |>
  model(stl = STL(Trips ~ season(window = 7))) |>
  components() |>
  autoplot()
```
...
```{r stlagain3, echo=TRUE, warning=FALSE, fig.width=8, fig.height=3}
holidays |>
...
## Decomposition subseries

```{r elecequip3}
dcmp <- holidays |> model(stl = STL(Trips)) |>
  components()
...
autoplot(dcmp) +
  autolayer(holidays, Trips, alpha=0.4)
```
