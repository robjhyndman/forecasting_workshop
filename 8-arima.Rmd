...
This is multiple regression with \textbf{lagged \emph{errors}} as predictors. \emph{Don't confuse with moving average smoothing!}
\end{block}
...
```{r popfc2, echo=TRUE, cache=TRUE, fig.height=2.8}
fit |>
  forecast(h = 10) |>
  filter(Country == "Australia") |>
...
```{r usmelec3, echo=TRUE, fig.height=2.8}
usmelec |>
  model(arima = ARIMA(log(Generation))) |>
  forecast(h = "3 years") |>
  autoplot(usmelec)
...
```{r usmelec4, echo=TRUE, fig.height=2.8}
usmelec |>
  model(arima = ARIMA(log(Generation))) |>
  forecast(h = "3 years") |>
...
fc |> filter(Region == "Snowy Mountains", Purpose == "Holiday") |>
